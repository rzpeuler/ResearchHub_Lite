...
import type { DocumentInputRef, DocumentParser, ResolvedDocumentInput } from './contracts.ts'
...
export class DocumentInputResolver {
  ...
  async resolve(input: DocumentInputRef): Promise<ResolvedDocumentInput> {
    ...
    const documentId = input.documentId ?? `document-${createHash('sha256').update(source.bytes).digest('hex').slice(0, 16)}`
  ...
  private async acquire(input: DocumentInputRef): Promise<{ bytes: Uint8Array; filename: string; mediaType: string }> {
    if (input.type === 'text') return { bytes: new TextEncoder().encode(input.text), filename: input.originalFilename ?? 'document.txt', mediaType: input.mediaType ?? 'text/plain' }
    if (input.type === 'bytes') {
      ...
      return { bytes: Uint8Array.from(input.bytes), filename, mediaType: input.mediaType ?? mediaTypeFor(filename) }
    }
    ...
    try {
      ...
      return { bytes, filename, mediaType: mediaTypeFor(filename) }
