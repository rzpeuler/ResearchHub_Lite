...
export class GdeltResearchPlugin implements ResearchAcquisitionPlugin {
  ...
  async discover(request: ResearchAcquisitionRequest): Promise<readonly ResearchSourceCandidate[]> {
    const isIndustry = request.industry !== undefined
    const target = isIndustry ? request.industry! : request.company
    const terms = isIndustry ? [request.industry!.name, ...(request.industry!.aliases ?? []), ...request.industry!.searchTerms].slice(0, 8).map((x) => x.trim()).filter(Boolean) : [request.company.name ?? request.company.symbol]
