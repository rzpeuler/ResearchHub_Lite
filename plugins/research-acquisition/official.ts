...
export class CninfoOfficialDisclosureClient implements OfficialDisclosureClient {
  ...
  async list(request: ResearchAcquisitionRequest): Promise<readonly OfficialDisclosureRecord[]> {
    ...
    const query = async (stock: string, searchkey: string) => { const form = new URLSearchParams({ stock, searchkey, pageNum: '1', pageSize: String(this.pageSize), tabName: 'fulltext', column }); const response = await this.request(this.endpoint, { method: 'POST', headers: { accept: 'application/json', 'content-type': 'application/x-www-form-urlencoded', 'user-agent': 'ResearchHub/PersonalResearchV1' }, body: form }); if (!response.ok) throw new Error(`CNINFO request failed with HTTP ${response.status}`); const payload = await response.json() as Record<string, unknown>; const rows = Array.isArray(payload.announcements) ? payload.announcements : []; return rows.flatMap((row) => { const record = CninfoOfficialDisclosureClient.record(row); return record && (!request.asOf || Date.parse(record.publishedAt) <= Date.parse(request.asOf)) ? [record] : [] }) }
    const exact = await query(request.company.symbol, ''); if (exact.length > 0 || !request.company.name) return exact; return query('', request.company.name)
  }
