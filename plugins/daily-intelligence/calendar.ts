...
export interface TradingDayResult { readonly tradeDate: string; readonly isTradingDay: boolean; readonly calendarConfidence: 'provider' | 'cache' | 'fallback' }
...
export class TradingCalendarService {
  ...
  async isTradingDay(date: string): Promise<TradingDayResult> { if (this.options.manualHolidays?.includes(date)) return { tradeDate: date, isTradingDay: false, calendarConfidence: 'provider' }; if (this.options.manualTradingDays?.includes(date)) return { tradeDate: date, isTradingDay: true, calendarConfidence: 'provider' }; if (this.options.provider) { try { const value = await this.options.provider(date); if (value !== undefined) { await this.save(date, value); return { tradeDate: date, isTradingDay: value, calendarConfidence: 'provider' } } } catch { /* fall through to cache */ } } const cached = await this.load(date); if (cached !== undefined) return { tradeDate: date, isTradingDay: cached, calendarConfidence: 'cache' }; const weekday = new Date(`${date}T00:00:00Z`).getUTCDay(); return { tradeDate: date, isTradingDay: weekday !== 0 && weekday !== 6, calendarConfidence: 'fallback' } }
