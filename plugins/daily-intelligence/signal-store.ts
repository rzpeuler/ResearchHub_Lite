...
import type { DailyResearchSignal, EventResearchSignalStore } from './contracts.ts'

export class FileDailySignalStore implements EventResearchSignalStore {
  ...
  async getById(signalId: string): Promise<DailyResearchSignal | undefined> { return (await this.read()).find((signal) => signal.signalId === signalId) }
