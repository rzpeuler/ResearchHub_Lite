...
export async function executeThesisFormalize(input: ThesisFormalizeSemanticInput, executor?: ReasoningExecutor): Promise<ThesisFormalizeSemanticResult> {
  ...
  const run = async (repair: boolean): Promise<FormalizedThesisResult> => {
    const response = await executor.execute({ operation: 'thesis_formalize_semantic', instruction: repair ? 'Repair the previous thesis formalization. Use only supplied evidence and return one bounded JSON object.' : 'Formalize the narrative into falsifiable thesis propositions. Generate candidate types, evidence basis, dependencies, verification conditions, and research gaps. Use only supplied evidence refs.', input: repair ? { context: boundedInput(input), previousOutput: boundedOutput(firstOutput), diagnostics: diagnostics.slice(0, 16) } : boundedInput(input), outputContract: contract() })
