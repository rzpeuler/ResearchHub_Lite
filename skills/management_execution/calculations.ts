...
function assess(commitment: ManagementCommitment, outcome: ManagementOutcome | undefined, asOf: string): ManagementExecutionAssessment {
  ...
  if (commitment.targetType === 'qualitative') {
    const assessment: ExecutionAssessment = outcome.qualitativeResult ?? 'inconclusive'
    return { commitmentId: commitment.id, targetMetric: commitment.targetMetric, targetPeriod: commitment.targetPeriod, assessment, ...(targetOutput === undefined ? {} : { target: targetOutput }), outcome: { ...(outcome.value === undefined ? {} : { value: outcome.value }), ...(outcome.unit === undefined ? {} : { unit: outcome.unit }), ...(outcome.statement === undefined ? {} : { statement: outcome.statement }) }, sourceRefs, diagnostics: [] }
  }
