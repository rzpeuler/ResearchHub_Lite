...
const categories: readonly DailyResearchSignal['category'][] = ['announcement', 'earnings', 'performance_forecast', 'investor_relations', 'institutional_research', 'institutional_activity', 'expectation_snapshot', 'expectation_revision', 'management_guidance', 'macro', 'market', 'community', 'technology', 'industry', 'news']
const structuredCategories: readonly DailyResearchSignal['category'][] = ['market', 'expectation_snapshot', 'expectation_revision', 'institutional_activity', 'industry']
function freezesCategory(signal: DailyResearchSignal): boolean { return signal.kind === 'market' || signal.kind === 'expectation' || signal.kind === 'institutional_activity' || signal.kind === 'industry_observation' || structuredCategories.includes(signal.category) }

export class DailyIntelligenceSignalEnrichmentSkill {
  enrich(signal: DailyResearchSignal, watchlistSymbols: readonly string[], focusTags: readonly string[]): DailySignalEnrichment { const value = `${signal.title} ${signal.excerpt ?? ''}`.toLowerCase(); const entities = [...new Set([...signal.entities, ...watchlistSymbols.filter((item) => value.includes(item.toLowerCase()))])].sort(); const themes = [...new Set([...signal.themes, ...focusTags.filter((item) => value.includes(item.toLowerCase()))])].sort(); const category = !freezesCategory(signal) && signal.category === 'news' && /公告|业绩|财报|年报|季报|investor|guidance/i.test(value) ? 'earnings' : signal.category; return { signal: { ...signal, entities, themes, category }, entities, themes, category, rationale: `bounded lexical enrichment; entities=${entities.length}; themes=${themes.length}` } }
  ...
  async enrichManyDetailedAsync(signals: readonly DailyResearchSignal[], watchlistSymbols: readonly string[], focusTags: readonly string[], executor?: ReasoningExecutor): Promise<DailyEnrichmentResult> {
    ...
    try {
      const result = await executor.execute({ operation: 'daily_signal_enrichment', instruction: 'Return a JSON object with a signals array. You may return only the signals you can classify confidently; each row must keep a supplied signalId and include at least one semantic field. Omitted or invalid semantic fields use deterministic fallback. Never modify identity, source, hashes, timestamps, or canonical IDs. For structured market, expectation, institutional-activity, and industry-observation signals, preserve the supplied category exactly; enrich themes, relevance, importance, sentiment, and narrative only.', input: { signals: lexical, watchlistSymbols, focusTags }, outputContract: { signals: [{ signalId: 'string', entities: ['watchlist-symbol'], themes: ['string'], category: 'announcement|earnings|performance_forecast|investor_relations|institutional_research|institutional_activity|expectation_snapshot|expectation_revision|management_guidance|macro|market|community|technology|industry|news', relevance: 0.0, sentiment: 0.0, importance: 0.0, narrative: 'string' }] }, metadata: { workflow: 'daily-intelligence' } })
      ...
      for (const raw of rows) {
        ...
        if (typeof row.category === 'string' && categories.includes(row.category as DailyResearchSignal['category']) && (!freezesCategory(lexical.find((signal) => signal.signalId === row.signalId)!) || row.category === lexical.find((signal) => signal.signalId === row.signalId)?.category)) patch.category = row.category as DailyResearchSignal['category']
