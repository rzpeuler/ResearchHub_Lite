...
export function buildExtractKnowledgeOutputContract(schema: CurationSchemaContext): StructuredOutputContract {
  ...
  const temporal = objectSchema(['asOf', 'scope'], { asOf: { type: ['string', 'null'], description: 'Actual Schema 0.3 date-like value or null; do not invent a date.' }, scope: objectSchema(['type', 'start', 'end', 'label'], { type: enumSchema(schema.claimTemporalScopeTypes), start: { type: ['string', 'null'], description: 'Actual Schema 0.3 date-like value or null; do not invent a date.' }, end: { type: ['string', 'null'], description: 'Actual Schema 0.3 date-like value or null; do not invent a date.' }, label: { type: ['string', 'null'], description: 'Semantic period labels belong here.' } }) })
