...
const SUPPORTED_SECURITY_EXCHANGES = new Set(Object.keys(EXCHANGE_ALIASES))
const PARENTHESIZED_SECURITY_SUFFIX = /^(?<base>.+?)\s*[（(]\s*(?<ticker>[0-9]{6})\.(?<exchange>[A-Za-z]{2,6})\s*[）)]\s*$/u
const BARE_SECURITY_SUFFIX = /^(?<base>.+?)\s+(?<ticker>[0-9]{6})\.(?<exchange>[A-Za-z]{2,6})\s*$/u
const SECURITY_LIKE_SUFFIX = /^[0-9]+\.[A-Za-z]+$/u
...
export function normalizeCompanyCandidateIdentity(candidate: EntityCandidate, allowedSemanticFields: readonly string[] = COMPANY_FIELDS): CompanyIdentityNormalizationResult {
  ...
  if (security?.groups?.base !== undefined && security.groups.ticker !== undefined && security.groups.exchange !== undefined && isSupportedSecurityExchange(security.groups.exchange)) {
  ...
  if (parentheses?.groups?.base !== undefined && parentheses.groups.label !== undefined) {
    ...
    if (ASCII_ALIAS.test(label) && !SECURITY_LIKE_SUFFIX.test(label)) { name = clean(parentheses.groups.base); bilingualAlias = label }
  }
