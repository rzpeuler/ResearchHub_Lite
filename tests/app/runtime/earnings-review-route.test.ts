...
test('ER34 HTTP review-earnings route starts authoritative Workflow', async () => {
  ...
  try { const info = await server.start(); const response = await fetch(`${info.origin}/api/production/review-earnings`, { method: 'POST', headers: { origin: info.origin, 'x-researchhub-runtime-token': info.runtimeToken, 'content-type': 'application/json' }, body: JSON.stringify({ symbol: '600519', exchange: 'SSE', fiscalYear: 2026, period: 'H1' }) }); assert.equal(response.status, 202); const body = await response.json() as { accepted: boolean; runId: string }; assert.equal(body.accepted, true); assert.equal(runtime.workflowService.getWorkflowStatus(body.runId)?.workflowType, 'earnings_review'); await new Promise((resolve) => setTimeout(resolve, 20)); assert.equal(runtime.workflowService.getWorkflowStatus(body.runId)?.status, 'blocked') } finally { await server.close(); await runtime.close(); await Promise.resolve((modelRuntime as unknown as { dispose?: () => void | Promise<void> }).dispose?.()).catch(() => undefined); await rm(root, { recursive: true, force: true }) }
})
