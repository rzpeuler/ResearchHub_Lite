...
async function writeGraphFixture(root: string): Promise<void> {
  ...
  await put('entities', 'industry-upstream', { id: 'entity:industry-upstream', type: 'industry', name: 'Materials', aliases: [], lifecycle: { status: 'active' } }, 'entity')
  await put('entities', 'industry-downstream', { id: 'entity:industry-downstream', type: 'industry', name: 'Equipment', aliases: [], lifecycle: { status: 'active' } }, 'entity')
  ...
  await put('relations', 'industry-upstream', { id: 'relation:industry-upstream', type: 'upstream_of', sourceRef: 'entity:industry-upstream', targetRef: 'entity:industry', lifecycle: { status: 'active' } }, 'relation')
  await put('relations', 'industry-downstream', { id: 'relation:industry-downstream', type: 'upstream_of', sourceRef: 'entity:industry', targetRef: 'entity:industry-downstream', lifecycle: { status: 'active' } }, 'relation')
...
test('KnowledgeGraphService returns bounded deterministic directory and rooted graph projections', async () => {
  ...
  try {
    ...
    assert.deepEqual(company.edges.map((edge) => edge.ref), ['relation:company-industry', 'relation:company-product', 'relation:company-tech', 'relation:industry-downstream', 'relation:industry-upstream', 'relation:theme-industry'])
    assert.equal((await service.getGraphProjection({ rootRef: 'entity:theme', depth: 1 })).profile, 'theme_context')
    const themeProfile = await service.getGraphProjection({ rootRef: 'entity:theme', depth: 1 })
    assert.ok(themeProfile.nodes.some((node) => node.ref === 'entity:industry'))
    assert.deepEqual(themeProfile.edges.map((edge) => [edge.sourceRef, edge.targetRef]), [['entity:theme', 'entity:industry']])
    const industry = await service.getGraphProjection({ rootRef: 'entity:industry', depth: 1 })
    assert.equal(industry.profile, 'industry_context')
    assert.ok(industry.edges.some((edge) => edge.ref === 'relation:industry-upstream' && edge.sourceRef === 'entity:industry-upstream' && edge.targetRef === 'entity:industry'))
    assert.ok(industry.edges.some((edge) => edge.ref === 'relation:industry-downstream' && edge.sourceRef === 'entity:industry' && edge.targetRef === 'entity:industry-downstream'))
    assert.ok(industry.nodes.some((node) => node.ref === 'entity:company'))
    const product = await service.getGraphProjection({ rootRef: 'entity:product', depth: 1 })
    assert.equal(product.profile, 'product_context')
    assert.ok(product.edges.some((edge) => edge.ref === 'relation:company-product' && edge.sourceRef === 'entity:company' && edge.targetRef === 'entity:product'))
    const technology = await service.getGraphProjection({ rootRef: 'entity:technology', depth: 1 })
    assert.equal(technology.profile, 'technology_context')
    assert.ok(technology.edges.some((edge) => edge.ref === 'relation:company-tech' && edge.sourceRef === 'entity:company' && edge.targetRef === 'entity:technology'))
    const companyOneHop = await service.getGraphProjection({ rootRef: 'entity:company', depth: 1 })
    assert.equal(companyOneHop.nodes.some((node) => node.ref === 'entity:theme'), false)
    assert.equal(company.nodes.some((node) => node.ref === 'entity:theme'), true)
...
test('KnowledgeGraphService rejects unsupported or inactive roots and clamps graph bounds', async () => {
  ...
  try {
    ...
    const defaults = await service.getGraphProjection({ rootRef: 'entity:company' })
    assert.equal(defaults.nodeLimit, 60)
    assert.equal(defaults.edgeLimit, 120)
    const hard = await service.getGraphProjection({ rootRef: 'entity:company', maxNodes: 999, maxEdges: 999 })
    assert.equal(hard.nodeLimit, 150)
    assert.equal(hard.edgeLimit, 300)
    assert.equal((await service.getDirectoryProjection()).companies.limit, 30)
    assert.equal((await service.getDirectoryProjection(999)).companies.limit, 100)
