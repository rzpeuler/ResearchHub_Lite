...
test('runtime registration has an explicit execution classification and deterministic binding', () => {
  ...
  assert.deepEqual(counts, { IMPLEMENTED: 11, PARTIAL: 12, PLANNED: 6 })
  assert.deepEqual(runtime.map((item) => item.id), ['business_driver_analysis', 'business_model_map', 'consensus_expectations_analysis', 'dcf_valuation', 'earnings_variance_analysis', 'estimate_revision_analysis', 'guidance_analysis', 'reverse_dcf_expectation_decode', 'scenario_valuation', 'thesis_red_team', 'unit_economics'])
