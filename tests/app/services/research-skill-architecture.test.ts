...
test('runtime registration has an explicit execution classification and deterministic binding', () => {
  ...
  assert.deepEqual(counts, { IMPLEMENTED: 15, PARTIAL: 9, PLANNED: 5 })
  assert.deepEqual(runtime.map((item) => item.id), ['business_driver_analysis', 'business_model_map', 'capital_allocation_review', 'consensus_expectations_analysis', 'dcf_valuation', 'earnings_variance_analysis', 'estimate_revision_analysis', 'financial_quality_analysis', 'guidance_analysis', 'management_execution', 'market_structure_analysis', 'reverse_dcf_expectation_decode', 'scenario_valuation', 'thesis_red_team', 'unit_economics'])
