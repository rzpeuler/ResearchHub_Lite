...
try {
  ...
  const evidence = { generatedAt, taskId: 'RHL-PERSONAL-RESEARCH-V1-M3A-EVENT-RESEARCH-001-FIX-002', classification: decision.classification, processExitCode: decision.exitCode, implementationCommit: 'pre-commit-working-tree', sourceBaseline: 'af9f473afe992f220bdb1cd0650454d6a448d798', workingTreeDiffCleanAfterGeneration: false, realPiReasoningExecutor: true, host: delegate.runtimeMetadata(), operations, gate: gateInput, run: { status: applicationResult.status, reportId: applicationResult.reportId ?? null, reportPath: applicationResult.reportPath ?? null, errors: applicationResult.errorSummary ?? null, diagnostics: telemetry.diagnostics ?? [], stageADiagnostics: telemetry.stageADiagnostics ?? [], stageBDiagnostics: telemetry.stageBDiagnostics ?? [], stageBFirstAttemptShape: telemetry.stageBFirstAttemptShape ?? null, stageBRepairAttemptShape: telemetry.stageBRepairAttemptShape ?? null, proposalRejectionDiagnostics: telemetry.proposalRejectionDiagnostics ?? [] }, verification: telemetry.verification ?? null, canonicalCounts: { entities: finalAssets.objects.filter((item) => item.kind === 'entity').length, sources: finalAssets.objects.filter((item) => item.kind === 'source').length, claims: finalAssets.objects.filter((item) => item.kind === 'claim').length }, reportContract: { persisted: gateInput.reportPersisted, workflowState: telemetry.reportPersistence, note: 'event_research is admitted by app/services/research-report.ts and persisted by the existing report writer; the Real Pi gate may still block on model contract fallback' }, irrelevantSourceCanonicalized, secretsIncluded: false, rawBodiesIncluded: false }
  await writeFile(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`, 'utf8')
  console.log(JSON.stringify(evidence, null, 2))
  if (decision.exitCode !== 0) process.exitCode = decision.exitCode
} catch (error) {
  const evidence = { generatedAt, taskId: 'RHL-PERSONAL-RESEARCH-V1-M3A-EVENT-RESEARCH-001-FIX-002', classification: 'NOT_EXECUTED / BLOCKED', processExitCode: 1, implementationCommit: 'pre-commit-working-tree', realPiReasoningExecutor: false, error: error instanceof Error ? error.message : String(error), secretsIncluded: false, rawBodiesIncluded: false }
  await writeFile(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`, 'utf8')
