...
import type { EventResearchWorkflowInput } from '../../workflows/event-research/contracts.ts'
...
function signal(value: ResearchSourceCandidate): DailyResearchSignal {
  return { signalId: 'pi-event-signal-001', kind: 'announcement', category: 'announcement', provider: 'fixture', source: value, publishedAt: '2026-09-08T01:00:00.000Z', discoveredAt: '2026-09-08T01:05:00.000Z', entities: ['600519'], themes: [], title: 'Fixture selected event', contentHash: sha256('fixture-selected-event'), relevance: 1, novelty: 1, importance: 1, sourceTier: 1, excerpt: 'A selected event requiring bounded verification.' }
}
...
try {
  ...
  const seed = await new KnowledgeProductionGateway(registry).submit({ handle, producerType: 'fixture_seed', producerRunId: 'pi-event-seed-001', schemaProfile: { schemaVersion: '0.4', storageFormatVersion: '1', requiresRawProvenance: true }, entity: { localKey: 'company', entityType: 'company', name: 'Fixture Company', aliases: ['600519'], semanticFields: { ticker: '600519', exchange: 'SSE' } }, proposals: [{ proposalId: 'seed-thesis', kind: 'claim', claimType: 'thesis', subjectKey: 'company', statement: 'The company has durable operating strength.', sourceCandidateIds: ['pi-event-seed'] }, { proposalId: 'seed-assumption', kind: 'claim', claimType: 'assumption', statement: 'Event impact remains bounded until corroborated.', subjectKey: 'company', sourceCandidateIds: ['pi-event-seed'] }], evidenceBindings: [{ localSourceId: 'pi-event-seed', source: seedSource }], asOf: nowValue, now: () => nowValue })
  ...
  const input: EventResearchWorkflowInput = { workflowRunId: 'pi-event-research-001', handle, company: { symbol: '600519', name: 'Fixture Company', exchange: 'SSE' }, anchor: { kind: 'daily_signal', signalId: 'pi-event-signal-001' }, signalStore, reportRoot, acquisitionPlugins: [fixturePlugin('official-cninfo', [official], contents), fixturePlugin('gdelt-news', [supporting, irrelevant], contents)], reasoningExecutor: executor, asOf, now: () => nowValue }
  const baseline = await readCanonicalV04Assets(kbRoot)
  const result = await runEventResearch(input)
  const finalAssets = await readCanonicalV04Assets(kbRoot)
  const reportPath = result.report === undefined ? undefined : join(reportRoot, result.report.outputPath)
  ...
  const gateInput = { executed: true, realPiReasoningExecutor: true, operations, assessmentCalled: result.telemetry.assessment.called, assessmentValidated: result.telemetry.assessment.validated, assessmentApplied: result.telemetry.assessment.applied, synthesisCalled: result.telemetry.synthesis.called, synthesisValidated: result.telemetry.synthesis.validated, synthesisApplied: result.telemetry.synthesis.applied, fallbackUsed: result.telemetry.assessment.fallbackUsed || result.telemetry.synthesis.fallbackUsed, strongVerification: result.verification?.strongVerification === true, reportType: result.report?.reportType ?? null, reportSectionCount: result.report?.sections.length ?? 0, reportPersisted: reportPath === undefined ? false : await fileExists(reportPath), acceptedProposalCount: result.telemetry.acceptedProposalCount, canonicalSourceCountDelta, canonicalClaimCountDelta, irrelevantSourceCanonicalized, workflowTerminalStatus: result.status }
  const decision = evaluateEventResearchPiGate(gateInput)
  const evidence = { generatedAt, taskId: 'RHL-PERSONAL-RESEARCH-V1-M3A-EVENT-RESEARCH-001', classification: decision.classification, processExitCode: decision.exitCode, implementationCommit: 'pre-commit-working-tree', sourceBaseline: '37b4a0e664441f0c3cdc4dc5bd42c238649a9dce', workingTreeDiffCleanAfterGeneration: false, realPiReasoningExecutor: true, host: delegate.runtimeMetadata(), operations, gate: gateInput, run: { status: result.status, reportId: result.report?.reportId ?? null, reportPath: result.report?.outputPath ?? null, errors: result.errors, diagnostics: result.diagnostics }, verification: result.verification ?? null, canonicalCounts: { entities: finalAssets.objects.filter((item) => item.kind === 'entity').length, sources: finalAssets.objects.filter((item) => item.kind === 'source').length, claims: finalAssets.objects.filter((item) => item.kind === 'claim').length }, reportContract: { persisted: gateInput.reportPersisted, workflowState: result.telemetry.reportPersistence, note: 'event_research is admitted by app/services/research-report.ts and persisted by the existing report writer; the Real Pi gate may still block on model contract fallback' }, irrelevantSourceCanonicalized, secretsIncluded: false, rawBodiesIncluded: false }
