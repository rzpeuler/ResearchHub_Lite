...
const evidencePath = join(evidenceDir, 'rhl-production-workflow-diagnosis-001.json')
const summaryPath = join(evidenceDir, 'RHL_PRODUCTION_WORKFLOW_DIAGNOSIS_001_SUMMARY.md')
const baselineExpected = 'f5676c81db18c1599ffa8459ad98dc0c9eb14a82'
...
async function main(): Promise<void> {
  ...
  const bytes = pdfBytes(); const evidence: Dict = { taskId: 'RHL-DIAGNOSE-PRODUCTION-WORKFLOW-FAILURE-001', startedAt: now(), baselineExpected, provider: 'zhipu-openapi', model: 'glm-5.3-flash', secretsIncluded: false, rawHiddenReasoningIncluded: false, credentialValuesExposed: false, authHeadersExposed: false }
  try {
    ...
    const bootstrap = objectBody(await requestJson(origin, '/api/bootstrap')); const runtimeToken = typeof bootstrap.runtimeToken === 'string' ? bootstrap.runtimeToken : ''; if (!runtimeToken) throw new Error('Bootstrap did not return a runtime token'); const attachment = await upload(origin, runtimeToken, bytes); const uploadId = String(attachment.attachmentId ?? ''); if (!uploadId) throw new Error('Attachment upload did not return an attachmentId'); const productionWorkspaceRoot = (runtime.productionService as unknown as { readonly workspaceRoot?: string }).workspaceRoot; const expectedAttachmentPath = join(workspaceRoot, 'uploads', uploadId, String(attachment.filename ?? '')); const attachmentService = (server as unknown as { readonly attachmentService?: { resolveAttachmentPath: (id: string) => Promise<string> } }).attachmentService; const resolvedAttachmentPath = attachmentService === undefined ? undefined : await attachmentService.resolveAttachmentPath(uploadId); evidence.attachment = { ...(evidence.attachment as Dict), attachmentId: uploadId, uploadSucceeded: true, controlledStorageContentMatches: attachment.sha256 === sha256(bytes) && Number(attachment.size) === bytes.byteLength, canonicalMutationByUpload: false }; evidence.pathBoundary = { runtimeProductionWorkspaceSame: productionWorkspaceRoot === runtime.workspaceRoot, attachmentInsideRuntimeWorkspace: inside(runtime.workspaceRoot, expectedAttachmentPath), attachmentInsideProductionWorkspace: productionWorkspaceRoot === undefined ? false : inside(productionWorkspaceRoot, expectedAttachmentPath), attachmentOutsideCanonicalKnowledge: !inside(kbRoot, expectedAttachmentPath), attachmentPathMatchesResolvedPath: resolvedAttachmentPath === expectedAttachmentPath, resolvedAttachmentInsideProductionWorkspace: productionWorkspaceRoot === undefined || resolvedAttachmentPath === undefined ? false : inside(productionWorkspaceRoot, resolvedAttachmentPath) }
