...
const evidence = { generatedAt: new Date().toISOString(), scenario: 'real-network multi-run company research', providersAttempted: ['cninfo', 'gdelt', 'akshare'], runs: results, final: { knowledgeBaseRevision: finalHandle.revision, canonicalCounts: { entities: assets.objects.filter((item) => item.kind === 'entity').length, sources: assets.objects.filter((item) => item.kind === 'source').length, claims: assets.objects.filter((item) => item.kind === 'claim').length }, sources, rawIntegrity } }
