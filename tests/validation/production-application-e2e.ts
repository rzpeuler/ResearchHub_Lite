...
async function writeSummary(evidence: Dict): Promise<void> {
  ...
  const lines = [
    ...
    `- Baseline: HEAD=${String(baseline.head ?? 'n/a')}; origin/main=${String(baseline.originMain ?? 'n/a')}`, `- Offline regression: ${JSON.stringify(offline)}`,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const sseController = new AbortController(); const sseResponse = await fetch(`${origin}/api/events`, { signal: sseController.signal, headers: { Origin: origin } }); assertCondition(sseResponse.status === 200 && sseResponse.headers.get('content-type')?.includes('text/event-stream'), 'SSE endpoint did not open as an event stream')
    ...
    assertCondition(assistantDeltas.length > 0 && eventText.includes(freeMarker) && normalizedSafe, 'Free Research did not produce a safe normalized assistant SSE response')
