...
test('FIX-015 offline evidence inputs preserve authoritative contracts and expected source sizes', () => {
  ...
  assert.equal(JSON.stringify(moduleContract).length, 6359)
  assert.equal(moduleTransport.fingerprint, '992306492c86ef0c')
  assert.equal(moduleTransport.bytes, 4676)
