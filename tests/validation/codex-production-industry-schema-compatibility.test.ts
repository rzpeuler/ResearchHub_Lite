...
test('FIX-015 offline evidence inputs preserve authoritative contracts and expected source sizes', () => {
  ...
  assert.equal(JSON.stringify(moduleContract).length, 6203)
  assert.equal(moduleTransport.fingerprint, '37f55ba9cafed372')
  assert.equal(moduleTransport.bytes, 4558)
