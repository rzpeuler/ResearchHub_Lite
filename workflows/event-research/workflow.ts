...
import type { ResearchAcquisitionDiagnostic, ResearchAcquisitionPlugin, ResearchCompanyIdentity, ResearchProviderOutcome, ResearchSourceCandidate } from '../../plugins/research-acquisition/contracts.ts'
...
import { EVENT_RESEARCH_REPORT_CONTRACT_NOTE, type EventAcquiredSource, type EventResearchReport, type EventResearchResolvedAnchor, type EventResearchTelemetry, type EventResearchWorkflowInput, type EventResearchWorkflowResult, type EventSourceRole } from './contracts.ts'
...
function isoDate(value: unknown): string | undefined { const raw = text(value); if (!raw) return undefined; if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) return raw; const time = Date.parse(raw); if (Number.isNaN(time)) return undefined; return new Date(time).toISOString().slice(0, 10) }
function validDateTime(value: unknown): boolean { const raw = text(value); return raw !== '' && !Number.isNaN(Date.parse(raw)) }
...
function emptyProvider(provider: string, reason: string): { readonly outcome: ResearchProviderOutcome; readonly diagnostic: ResearchAcquisitionDiagnostic } { return { outcome: { provider, providerAttempted: false, providerSucceeded: false, providerEmpty: true, providerFailed: false, usableSourceCount: 0 }, diagnostic: { provider, status: 'empty', reason } } }
...
interface AcquisitionResult { readonly sources: readonly EventAcquiredSource[]; readonly outcomes: readonly ResearchProviderOutcome[]; readonly diagnostics: readonly ResearchAcquisitionDiagnostic[]; readonly discoveredCount: number; readonly selectedCount: number; readonly futureFilteredCount: number; readonly outsideWindowFilteredCount: number; readonly unknownDateCount: number; readonly deduplicatedCount: number }
...
async function acquireEventSources(input: EventResearchWorkflowInput, company: ResearchCompanyIdentity, context: EventResearchAnchorContext, asOf: string, windowDays: number): Promise<AcquisitionResult> {
  const diagnostics: ResearchAcquisitionDiagnostic[] = []; const outcomes: ResearchProviderOutcome[] = []; const acquired: EventAcquiredSource[] = []; const urls = new Set<string>(); const hashes = new Set<string>(); let discoveredCount = 0; let selectedCount = 0; let futureFilteredCount = 0; let outsideWindowFilteredCount = 0; let unknownDateCount = 0; let deduplicatedCount = 0
  for (const provider of PROVIDERS) {
    ...
    try { discovered = await plugin.discover({ company, asOf, limitPerKind: 6 }); discoveredCount += discovered.length } catch (error) { const reason = error instanceof Error ? error.message : String(error); outcomes.push({ provider, providerAttempted: true, providerSucceeded: false, providerEmpty: false, providerFailed: true, usableSourceCount: 0 }); diagnostics.push({ provider, status: 'failed', reason }); continue }
    ...
    let usable = 0; let failed = false
    for (const candidate of candidates) {
      if (acquired.length >= 12) break
      abortIfNeeded(input.signal)
      try {
        const fetched = await plugin.fetch(candidate); const payload = validateUsableAcquisitionPayload(fetched.content); if (payload.status !== 'usable') { diagnostics.push({ provider, candidateId: candidate.candidateId, kind: candidate.kind, status: payload.status, reason: payload.reason }); continue }
    ...
    outcomes.push({ provider, providerAttempted: true, providerSucceeded: usable > 0, providerEmpty: discovered.length === 0 || (candidates.length === 0 && !failed), providerFailed: failed, usableSourceCount: usable })
  }
