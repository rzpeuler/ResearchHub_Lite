...
export function buildEarningsExpectationAnalysis(input: EarningsExpectationIntegrationInput): EarningsExpectationAnalysis {
  ...
  const resultCutoff = input.resultPublishedAt ?? bundle.resultPublishedAt; const resultTime = resultCutoff === undefined ? undefined : timestamp(resultCutoff)
  if (resultCutoff !== undefined && (resultTime === undefined || resultTime > timestamp(input.analysisAsOf)!)) diagnostics.push(resultTime === undefined ? 'resultPublishedAt_invalid' : 'resultPublishedAt_invalid')
  ...
  for (const actual of actuals) {
    ...
    for (const institutionKey of uniqueSorted(bundle.priorEstimateInstitutionKeys ?? [])) {
      ...
      const prior = selectPriorEstimate({ estimates: [...estimateMap.values()], metric: actual.metric, fiscalPeriod: actual.fiscalPeriod, institutionKey, beforePublishedAt: resultCutoff!, analysisAsOf: input.analysisAsOf }); const result = compareActualVsPriorEstimate({ actual, estimates: [...estimateMap.values()], institutionKey, comparisonCutoff: resultCutoff! }); if (result !== undefined && prior !== undefined) actualVsPriorEstimate.push({ result, sourceCandidateIds: sourceIds(actual.sourceCandidateIds, prior.sourceCandidateIds) }); else diagnostics.push(`prior_estimate_unavailable:${institutionKey}:${actual.metric}`)
    }
  }
  const estimateRevisions: SourcedEstimateRevision[] = []; const revisionTargets = new Map<string, string[]>()
  const seenRevisionLinks = new Set<string>();
  for (const link of bundle.estimateRevisionLinks ?? []) { const oldId = text(link.oldEstimateId) ? link.oldEstimateId.trim() : ''; const newId = text(link.newEstimateId) ? link.newEstimateId.trim() : ''; const linkKey = `${oldId}->${newId}`; if (!newId || seenRevisionLinks.has(linkKey)) continue; seenRevisionLinks.add(linkKey); revisionTargets.set(newId, [...(revisionTargets.get(newId) ?? []), oldId]) }
  for (const [newId, oldIds] of revisionTargets) { if (oldIds.length > 1) { diagnostics.push(`ambiguous_revision_target:${newId}`); continue }; const oldEstimate = estimateMap.get(oldIds[0]!); const newEstimate = estimateMap.get(newId); const result = oldEstimate !== undefined && newEstimate !== undefined ? buildEstimateRevisionBridge({ oldEstimate, newEstimate }) : undefined; if (result === undefined) diagnostics.push(`estimate_revision_invalid:${oldIds[0] ?? 'unknown'}->${newId}`); else estimateRevisions.push({ result, sourceCandidateIds: sourceIds(oldEstimate!.sourceCandidateIds, newEstimate!.sourceCandidateIds) }) }
  ...
  const comparisons = uniqueResults(sort(actualVsConsensus, (item) => key(item.result.metric, item.result.fiscalPeriod, item.result.benchmark)), (item) => JSON.stringify(item.result)); const priorComparisons = uniqueResults(sort(actualVsPriorEstimate, (item) => key(item.result.metric, item.result.fiscalPeriod, item.result.benchmark, item.result.direction)), (item) => JSON.stringify(item.result)); const revisions = uniqueResults(sort(estimateRevisions, (item) => key(item.result.institutionKey, item.result.metric, item.result.fiscalPeriod, item.result.oldPublishedAt, item.result.newPublishedAt)), (item) => JSON.stringify(item.result)); const guidanceRevisionResults = uniqueResults(sort(guidanceRevisions, (item) => key(item.result.metric, item.result.fiscalPeriod, item.result.oldGuidanceId, item.result.newGuidanceId)), (item) => JSON.stringify(item.result)); const guidanceConsensusResults = uniqueResults(sort(guidanceVsConsensus, (item) => key(item.result.metric, item.result.fiscalPeriod, item.result.guidanceId, item.result.consensusAsOf)), (item) => JSON.stringify(item.result)); const segmentResults = uniqueResults(sort(segmentKpiDeltas, (item) => key(item.result.segmentKey, item.result.metric, item.result.currentPeriod)), (item) => JSON.stringify(item.result));
...
export function enrichEarningsReviewSectionsWithExpectations(sections: readonly EarningsReviewSection[], analysis: EarningsExpectationAnalysis): readonly EarningsReviewSection[] {
  ...
  const actual = analysis.actualVsConsensus.map((item) => `${item.result.metric} ${item.result.fiscalPeriod}: actual ${item.result.actual} vs PIT consensus ${item.result.benchmark} (${item.result.direction})`); const prior = analysis.actualVsPriorEstimate.map((item) => `${item.result.metric} ${item.result.fiscalPeriod}: actual ${item.result.actual} vs selected prior estimate ${item.result.benchmark} (${item.result.direction})`)
  const revisions = analysis.estimateRevisions.map((item) => `${item.result.institutionKey} ${item.result.metric} ${item.result.fiscalPeriod}: ${item.result.oldValue} -> ${item.result.newValue} (delta ${item.result.absoluteRevision})`); const guidanceRevisionText = analysis.guidanceRevisions.map((item) => `${item.result.metric} ${item.result.fiscalPeriod}: ${JSON.stringify(item.result)}`); const guidanceText = analysis.currentGuidance.map((item) => item.guidanceType === 'qualitative' ? `${item.guidanceId} ${item.metric} ${item.fiscalPeriod}: qualitative; qualifiers=${item.qualifiers.join(', ')}; published=${item.publishedAt}` : `${item.guidanceId} ${item.metric} ${item.fiscalPeriod}: low=${item.low ?? 'n/a'}, high=${item.high ?? 'n/a'}, midpoint=${item.midpoint ?? 'n/a'}, range width=${item.low !== undefined && item.high !== undefined ? item.high - item.low : 'n/a'}`); const guidanceConsensusText = analysis.guidanceVsConsensus.map((item) => `${item.result.guidanceId} vs PIT consensus ${item.result.consensusMean}: ${item.result.relationship}`); const segments = analysis.segmentKpiDeltas.map((item) => `${item.result.segmentKey} ${item.result.metric} ${item.result.currentPeriod}: current=${item.result.currentValue}${item.result.priorComparison ? `; prior=${item.result.priorComparison.benchmark} (${item.result.priorComparison.direction})` : ''}${item.result.expectationComparison ? `; expectation=${item.result.expectationComparison.benchmark} (${item.result.expectationComparison.direction})` : ''}`)
  const result = sections.map((section) => {
    ...
    if (section.title === 'Valuation Implications' && analysis.actualVsConsensus.length > 0) { const markdown = section.markdown.replace('Consensus unavailable', '').trim(); return { ...append({ ...section, markdown }, `### Expectation availability\nPoint-in-time consensus comparison is available for ${analysis.actualVsConsensus.length} current-period metric(s).\nValuation impact is not calculated in W2-004.`, sourceIdsFor(analysis.actualVsConsensus)), markdown: append({ ...section, markdown }, `### Expectation availability\nPoint-in-time consensus comparison is available for ${analysis.actualVsConsensus.length} current-period metric(s).\nValuation impact is not calculated in W2-004.`, sourceIdsFor(analysis.actualVsConsensus)).markdown } }
    return section
  })
