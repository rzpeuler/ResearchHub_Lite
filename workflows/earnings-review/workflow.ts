...
function reportSections(sections: readonly EarningsReviewSection[], sources: readonly NormalizedResearchSource[], outcomeSources: Readonly<Record<string, string>>, outcomeClaims: Readonly<Record<string, string>>): ResearchReportSection[] {
  return sections.map((section) => ({ id: section.id, title: section.title, markdown: section.markdown, sourceRefs: section.sourceCandidateIds.map((id) => outcomeSources[id]).filter((id): id is string => id !== undefined), claimRefs: section.assessmentRefs.flatMap((assessmentId) => Object.keys(outcomeClaims).filter((proposalId) => proposalId === assessmentId).map((proposalId) => outcomeClaims[proposalId]!)), evidenceLinks: section.sourceCandidateIds.flatMap((id) => { const source = sources.find((item) => item.candidate.candidateId === id); return source?.candidate.url ? [source.candidate.url] : [`evidence:${id}`] }) }))
}
...
export async function runEarningsReview(input: EarningsReviewWorkflowInput): Promise<EarningsReviewWorkflowResult> {
  ...
  try {
    ...
    const gateway = new KnowledgeProductionGateway(new KnowledgeBaseRegistry()); let outcome: KnowledgeProductionOutcome = { status: 'no_changes', knowledgeBaseId: input.handle.knowledgeBaseId, knowledgeBaseRevision: input.handle.revision, baseRevision: input.handle.revision, createdIds: [], updatedIds: [], sourceRefsByLocalId: {}, claimRefsByProposalId: {}, entityRefsByLocalKey: {}, resolutionIntents: [], errors: [] }
