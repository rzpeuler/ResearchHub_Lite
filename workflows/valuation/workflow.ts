...
export async function runValuation(input: ValuationWorkflowInput): Promise<ValuationWorkflowResult> {
  ...
  try {
    ...
    if (basis && plan && computation && proposals.length > 0) {
      ...
      if (outcome.status === 'blocked' || outcome.status === 'failed') return { workflowRunId: input.workflowRunId, status: 'blocked', knowledgeBaseId: input.handle.knowledgeBaseId, knowledgeBaseRevision: outcome.knowledgeBaseRevision, proposalIds: gatewayProposals.map((proposal) => proposal.proposalId), committedIds: [], sourceIds: Object.values(outcome.sourceRefsByLocalId), claimIds: Object.values(outcome.claimRefsByProposalId), errors: outcome.errors, diagnostics: acquired.diagnostics, providerOutcome, basis, plan, computation, synthesis: synthesisOutput, telemetry, ...(compsResult === undefined ? {} : { compsResult }), qualityGate, crosscheck }
    }
    ...
    return { workflowRunId: input.workflowRunId, status: 'completed', knowledgeBaseId: input.handle.knowledgeBaseId, knowledgeBaseRevision: outcome.knowledgeBaseRevision, report: { reportId, outputPath }, proposalIds: proposals.map((proposal) => proposal.proposalId), committedIds: [...outcome.createdIds, ...outcome.updatedIds], sourceIds: Object.values(outcome.sourceRefsByLocalId), claimIds: Object.values(outcome.claimRefsByProposalId), errors: [], diagnostics: acquired.diagnostics, providerOutcome, basis, plan, computation, synthesis: synthesisOutput, telemetry, ...(compsResult === undefined ? {} : { compsResult }), qualityGate, crosscheck }
  } catch (error) {
