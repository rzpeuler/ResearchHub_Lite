...
import type { ValidatedExtractKnowledgeResult } from '../../skills/knowledge-curation/contracts.ts'
...
const defaults = { maxExtractionUnits: 64, maxPlanAttempts: 2, maxExtractionAttempts: 2, maxConcurrency: 4, maxResolutionAttempts: 2, maxResolutionCases: 32 }
export type EffectiveConfig = { maxExtractionUnits: number; maxPlanAttempts: number; maxExtractionAttempts: number; maxConcurrency: number; maxResolutionAttempts?: number; maxResolutionCases?: number; maxContextTokens?: number }
...
export async function runRawDocumentKnowledgeIngestion(input: RawDocumentKnowledgeIngestionInput): Promise<IngestionWorkflowResult> {
  ...
  try {
    ...
    const resolution = await resolveKnowledge({ assets, groups: consolidated.groups, reportMap: planned.reportMap, plan: acceptedPlan, rawRef, skill: input.skill, instructions: input.instructions, maxResolutionAttempts: config.maxResolutionAttempts, maxResolutionCases: config.maxResolutionCases, maxContextTokens: config.maxContextTokens, consolidationReviews: consolidated.reviewConstraints })
    ...
    if (resolution.blocked || planning.blocked) return { ...emptyResult(input, [...resolution.errors, ...(planning.errors ?? [])], planAttempts), knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, acceptedPlan, planAttempts, unitSummaries, candidateCounts: consolidated.candidateCounts, rejectedCandidates: consolidated.rejected, reviewItems: planning.reviewItems, reviewSummary, resolutionSummary: resolution.summary, extractionConcurrency, peakExtractionConcurrency: extraction.peak }
    if (!planning.changeSet) {
      ...
      if (noOp.kind === 'conflict') return { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, status: 'blocked', acceptedPlan, planAttempts, unitSummaries, candidateCounts: consolidated.candidateCounts, rejectedCandidates: consolidated.rejected, reviewItems: planning.reviewItems, reviewSummary, resolutionSummary: resolution.summary, writeStatus: 'no_changes', baseRevision: handle.revision, committedRevision: handle.revision, validationSummary: finalValidation, extractionConcurrency, peakExtractionConcurrency: extraction.peak, errors: [noOp.message ?? 'Execution log conflict'] }
      const persisted = noOp.record
      const persistedStatus = replayStatus(persisted.status) ?? record.status
      return { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, status: persistedStatus, acceptedPlan, planAttempts, unitSummaries, candidateCounts: consolidated.candidateCounts, rejectedCandidates: consolidated.rejected, reviewItems: planning.reviewItems, reviewSummary: reviewSummaryFromRecord(persisted), resolutionSummary: resolution.summary, writeStatus: noOp.kind === 'replay' ? 'already_committed' : 'no_changes', baseRevision: Number(persisted.baseRevision ?? handle.revision), committedRevision: Number(persisted.committedRevision ?? handle.revision), validationSummary: finalValidation, extractionConcurrency, peakExtractionConcurrency: extraction.peak, errors: finalValidation.errors.map((item) => item.message) }
    }
