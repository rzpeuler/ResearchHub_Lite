...
export async function runRawDocumentKnowledgeIngestion(input: RawDocumentKnowledgeIngestionInput): Promise<IngestionWorkflowResult> {
  ...
  try {
    ...
    if (!planning.changeSet) {
      const finalValidation = await validateKnowledgeBaseV03(handle.rootRef)
      if (reviewCases.length > 0) { const persistedCases = await persistReviewCases({ rootRef: handle.rootRef, knowledgeBaseId: handle.knowledgeBaseId, producerRunId: input.workflowRunId, cases: reviewCases, createdAt: reviewCasesCreatedAt, knowledgeBaseRevisionAtCreation: handle.revision }); if (persistedCases.kind === 'conflict') return { ...emptyResult(input, [persistedCases.message ?? 'ReviewCase persistence conflict'], planAttempts), knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, acceptedPlan, planAttempts, unitSummaries, candidateCounts: consolidated.candidateCounts, rejectedCandidates: consolidated.rejected, reviewItems: planning.reviewItems, reviewSummary, reviewCases, resolutionSummary: resolution.summary, extractionConcurrency, peakExtractionConcurrency: extraction.peak } }
      const record = { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, workflowInputFingerprint: fingerprint, status: terminalStatus(finalValidation, reviewSummary), writeStatus: 'no_changes' as const, baseRevision: handle.revision, committedRevision: handle.revision, reviewSummary, ...reviewCaseContext, completedAt: clock(), errors: finalValidation.errors.map((item) => item.message) }
