...
import { planKnowledgeChangeSet } from './changeset/changeset-planner.ts'
...
import { resolveKnowledge } from './resolution/knowledge-resolution.ts'
import { emptyReviewSummary, normalizeReviewSummary, reviewSummaryFromRecord, writeNoOpExecutionRecord } from './review/review-telemetry.ts'
import { buildReviewCases } from '../../knowledge/review/case-builder.ts'
import { persistReviewCases, recoverReviewCasesFromExecutionLog } from '../../knowledge/review/store.ts'
...
function reviewTerminalStatus(durableReviewCaseCount: number): 'completed' | 'completed_with_review' { return durableReviewCaseCount > 0 ? 'completed_with_review' : 'completed' }
function terminalStatus(validation: { readonly status: string }, durableReviewCaseCount: number): IngestionWorkflowResult['status'] { return validation.status === 'failed' ? 'blocked' : reviewTerminalStatus(durableReviewCaseCount) }
function replayTerminalStatus(validation: { readonly status: string }, historicalStatus: ReturnType<typeof replayStatus>, durableReviewCaseCount: number): IngestionWorkflowResult['status'] {
  if (validation.status === 'failed' || historicalStatus === 'blocked') return 'blocked'
  return terminalStatus(validation, durableReviewCaseCount)
}
  ...
  try {
    ...
    try {
      ...
      if (isRecord(replayLog) && replayStatus(replayLog.status)) {
        ...
        const replayReviewCases = await recoverReviewCasesFromExecutionLog(handle.rootRef, replayLog)
        const reviewSummary = reviewSummaryFromRecord(isRecord(context.reviewSummary) ? context : replayLog)
        const status = replayTerminalStatus(finalValidation, replayStatus(replayLog.status), replayReviewCases.length)
        return { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId: typeof context.documentId === 'string' ? context.documentId : undefined, status, unitSummaries: [], candidateCounts: {}, rejectedCandidates: [], reviewItems: [], reviewSummary, reviewCases: replayReviewCases, planAttempts: [], changeSetId: typeof replayLog.changeSetId === 'string' ? replayLog.changeSetId : undefined, writeStatus: 'already_committed', baseRevision: Number(replayLog.baseRevision ?? replayLog.committedRevision ?? handle.revision), committedRevision: Number(replayLog.committedRevision ?? handle.revision), validationSummary: finalValidation, errors: finalValidation.errors.map((item) => item.message) }
      }
    } catch (error) {
    ...
    const reviewCases = buildReviewCases({ knowledgeBaseId: handle.knowledgeBaseId, producerRunId: input.workflowRunId, createdAt: reviewCasesCreatedAt, rawRef, documentId, knowledgeBaseRevisionAtCreation: handle.revision, assets, groups: consolidated.groups, reviewItems: planning.reviewItems, consolidationReviews: consolidated.reviewConstraints, bindings: resolution.bindings, intents: resolution.intents, potentialNewInvestmentThemes: resolution.potentialNewInvestmentThemes })
    const reviewCaseSetHash = hashKnowledgeObject(reviewCases)
    const reviewCaseContext = { producerType: 'raw_document_knowledge_ingestion' as const, producerRunId: input.workflowRunId, reviewCases, reviewCaseIds: reviewCases.map((item) => item.reviewCaseId), reviewCaseSetHash, reviewCasesCreatedAt, knowledgeBaseRevisionAtCreation: handle.revision }
    const persistBlockedReviewCases = async (errors: readonly string[]): Promise<void> => {
      const logStore = new KnowledgeIngestionLogStore()
      await logStore.writeBlocked(handle, { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, status: 'blocked', rawRef, workflowInputFingerprint: fingerprint, documentId, baseRevision: handle.revision, committedRevision: handle.revision, reviewSummary, ...reviewCaseContext, errors: errors.map((message) => ({ code: 'review_blocked', message })) })
      if (reviewCases.length === 0) return
      const persisted = await persistReviewCases({ rootRef: handle.rootRef, knowledgeBaseId: handle.knowledgeBaseId, producerRunId: input.workflowRunId, cases: reviewCases, createdAt: reviewCasesCreatedAt, knowledgeBaseRevisionAtCreation: handle.revision })
      if (persisted.kind === 'conflict') throw new Error(persisted.message ?? 'ReviewCase persistence conflict')
    }
    if (resolution.blocked || planning.blocked) {
      await persistBlockedReviewCases([...resolution.errors, ...(planning.errors ?? [])])
      return { ...emptyResult(input, [...resolution.errors, ...(planning.errors ?? [])], planAttempts), knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, acceptedPlan, planAttempts, unitSummaries, candidateCounts: consolidated.candidateCounts, rejectedCandidates: consolidated.rejected, reviewItems: planning.reviewItems, reviewSummary, reviewCases, resolutionSummary: resolution.summary, potentialNewInvestmentThemes: resolution.potentialNewInvestmentThemes, recommendedNewInvestmentThemes: resolution.recommendedNewInvestmentThemes, extractionConcurrency, peakExtractionConcurrency: extraction.peak }
    }
    if (!planning.changeSet) {
      const finalValidation = await validateKnowledgeBaseV03(handle.rootRef)
      const record = { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, workflowInputFingerprint: fingerprint, status: terminalStatus(finalValidation, reviewCases.length), writeStatus: 'no_changes' as const, baseRevision: handle.revision, committedRevision: handle.revision, reviewSummary, ...reviewCaseContext, completedAt: clock(), errors: finalValidation.errors.map((item) => item.message) }
    ...
    assertNotAborted(input)
    const write = await (input.writer ?? ((targetHandle, payload) => writeKnowledgeBase(targetHandle, payload.receipt as never, { registry, clock, stagedStateValidator: async (rootRef) => { const staged = await validateKnowledgeBaseV03(rootRef); if (staged.status === 'failed') throw new Error(staged.errors.map((item) => item.message).join('; ')) } })))(handle, { receipt: changeSetValidation.validatedChangeSet, registry, clock, stagedStateValidator: async (rootRef) => { const staged = await validateKnowledgeBaseV03(rootRef); if (staged.status === 'failed') throw new Error(staged.errors.map((item) => item.message).join('; ')) } })
    ...
    const finalValidation = await validateKnowledgeBaseV03(handle.rootRef)
    return { workflowRunId: input.workflowRunId, knowledgeBaseId: handle.knowledgeBaseId, rawRef, documentId, status: terminalStatus(finalValidation, reviewCases.length), acceptedPlan, planAttempts, unitSummaries, candidateCounts: consolidated.candidateCounts, rejectedCandidates: consolidated.rejected, reviewItems: planning.reviewItems, reviewSummary, reviewCases, resolutionSummary: resolution.summary, potentialNewInvestmentThemes: resolution.potentialNewInvestmentThemes, recommendedNewInvestmentThemes: resolution.recommendedNewInvestmentThemes, changeSetId: changeSet.changeSetId, writeStatus: write.status, baseRevision: write.baseRevision, committedRevision: write.committedRevision, validationSummary: finalValidation, extractionConcurrency, peakExtractionConcurrency: extraction.peak, errors: finalValidation.errors.map((item) => item.message) }
  } catch (error) {
