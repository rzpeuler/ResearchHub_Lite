...
if (!enabled) {
  ...
  const cninfo = new CninfoOfficialDisclosureClient({ timeoutMs: 20_000 })
  ...
  for (const symbol of targets) {
    ...
    if (row) {
      try { publication = await cninfo.resolveAnnualReportPublication({ company: { symbol }, fiscalYear: row.basisFiscalYear, asOf: now }); stages.cninfoPublication = publication ? { status: 'FOUND', sourceUrl: (publication as { sourceUrl: string }).sourceUrl, officialPublishedAt: (publication as { officialPublishedAt: string }).officialPublishedAt } : { status: 'NOT_FOUND' } } catch (error) { stages.cninfoPublication = { status: 'UNAVAILABLE', error: error instanceof Error ? error.message : String(error) } }
    ...
    if (symbol === '600519') {
      ...
      for (const [window, asOf] of [['beforePublication', '2025-03-01T00:00:00.000Z'], ['afterPublication', '2025-04-10T00:00:00.000Z']] as const) {
        try {
          ...
          historicalPublication[window] = historical === undefined
            ? { status: 'NOT_FOUND', pitClassification: 'DATA_NOT_PUBLISHED' }
            : { status: 'FOUND', officialPublishedAt: historical.officialPublishedAt, sourceUrl: historical.sourceUrl, pitClassification: 'PUBLICATION_VERIFIED_VALUE_VERSION_UNVERIFIED', strictHistoricalBasisEligible: false }
        } catch (error) { historicalPublication[window] = { status: 'UNAVAILABLE', error: error instanceof Error ? error.message : String(error) } }
      }
