...
async function main() {
  ...
  try {
    ...
    const a = dispatch.start({ query: '600519 2026 年半年报', mode: { type: 'workflow', workflowId: 'earnings_review' }, contextPolicy: { structuredKnowledge: true, sourceLibrary: true }, persistencePolicy: { writeKnowledge: false } }); await a.completion; const aBundle = await dispatch.getBundle(`research-bundle-${a.runId!}`)
    const b = dispatch.start({ query: '研究 600519 2026 年半年报' }); await b.completion; const bBundle = await dispatch.getBundle(`research-bundle-${b.runId!}`)
