...
export function bootstrapPlan(platform = process.platform, paths = { venv: 'managed-venv', models: 'managed-models' }, basePython = candidatePlan(platform)[0]) {
  ...
  return { venv: { executable: python, args: [...baseArgs, '-m', 'venv', paths.venv], systemSitePackages: false }, install: { executable: join(paths.venv, platform === 'win32' ? 'Scripts' : 'bin', 'python' + (platform === 'win32' ? '.exe' : '')), args: ['-m', 'pip', 'install', '--disable-pip-version-check', '-r', requirementsRelativePath] }, models: { executable: tools, args: ['models', 'download', ...MODEL_FAMILIES, '--output-dir', paths.models] } }
}
...
export async function setup({ root = repoRoot, platform = process.platform, explicitPython, execute = runCommand, move = rename } = {}) {
  ...
  const stagingParent = resolve(root, '.researchhub-document-parser', '.staging'); await mkdir(stagingParent, { recursive: true }); const stagingRoot = await mkdtemp(join(stagingParent, 'run-')); const stagedVenv = join(stagingRoot, 'venv'); const stagedModels = join(stagingRoot, 'models'); const stagedPython = platform === 'win32' ? join(stagedVenv, 'Scripts', 'python.exe') : join(stagedVenv, 'bin', 'python'); const finalRoot = resolve(root, '.researchhub-document-parser'); const finalVenv = join(finalRoot, 'venv'); const finalModels = join(finalRoot, 'models'); const backupRoot = join(stagingRoot, 'backup'); const moved = { venv: false, models: false, stagedVenv: false, stagedModels: false }
  try {
    ...
    const stagedProbe = await probe(stagedPython, [], execute); if (!stagedProbe || !stagedProbe.doclingImportAvailable || stagedProbe.doclingVersion !== DOCLING_VERSION) return { status: 'INCONCLUSIVE', reason: 'PIP_INSTALL_FAILED' }
    ...
    if (!await nonEmptyDirectory(stagedModels) || !await smoke(stagedPython, stagedModels, root, execute)) return { status: 'INCONCLUSIVE', reason: 'BRIDGE_SMOKE_FAILED' }
    await mkdir(backupRoot, { recursive: true });
    try {
      if (await exists(finalVenv)) { await move(finalVenv, join(backupRoot, 'venv')); moved.venv = true }
      if (await exists(finalModels)) { await move(finalModels, join(backupRoot, 'models')); moved.models = true }
      await move(stagedVenv, finalVenv); moved.stagedVenv = true
      await move(stagedModels, finalModels); moved.stagedModels = true
      const verified = await readyState(root, platform, execute)
      if (verified.status === 'READY') return verified
      throw new Error('final verification failed')
    } catch {
      if (moved.stagedModels) await rm(finalModels, { recursive: true, force: true })
      if (moved.stagedVenv) await rm(finalVenv, { recursive: true, force: true })
      const restore = async (source, destination) => { try { if (await exists(source)) await move(source, destination) } catch {} }
      ...
      return { status: 'INCONCLUSIVE', reason: 'PROMOTION_FAILED' }
    }
