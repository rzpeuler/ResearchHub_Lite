...
async function parserBridgeDiagnostic(parser: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    try {
      ...
      return { preflightCommand: 'npm run document-parser:check', command: `managed-python ${bridgeRelative} <temporary-pdf>`, exitCode: 0, stderrSummary: '', stdoutSummary: 'bridge returned structured output', pythonExecutable: pythonRelative, doclingVersion, modelArtifactPath: modelsRelative, ...parser }
    } catch (error) {
      const detail = error as { readonly code?: number | string; readonly stderr?: string; readonly stdout?: string }
      return { preflightCommand: 'npm run document-parser:check', command: `managed-python ${bridgeRelative} <temporary-pdf>`, exitCode: typeof detail.code === 'number' ? detail.code : null, stderrSummary: diagnosticText(detail.stderr), stdoutSummary: diagnosticText(detail.stdout), bridgeInvocationFailure: diagnosticText(detail.stderr) || (parser.status === 'MANAGED_PYTHON_MISSING' ? 'managed parser Python executable is missing; bridge not invoked' : 'bridge exited non-zero'), pythonExecutable: pythonRelative, doclingVersion, modelArtifactPath: modelsRelative, ...parser }
    }
  } finally { await rm(temp, { recursive: true, force: true }) }
}
