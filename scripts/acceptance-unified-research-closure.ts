...
async function main(): Promise<void> {
  ...
  try {
    ...
    const methodologyPath = join(temp, 'closure-methodology', 'SKILL.md'); await mkdir(join(temp, 'closure-methodology'), { recursive: true }); await writeFile(methodologyPath, '# Closure methodology\nMANDATORY_METHOD_STEP_XYZ\n', 'utf8'); const skillRegistry = new ResearchSkillRegistry([{ id: 'closure-methodology', kind: 'research', researchCapability: 'custom_research', intentDescription: 'Closure methodology', whenToUse: 'Use the supplied closure methodology.', outputContract: 'ResearchBundle with evidence references', methodologySource: { type: 'researchhub_skill', path: methodologyPath }, enabled: true, scope: 'researchhub' }])
    ...
    assert.match(promptContext, /closure-methodology/); assert.match(promptContext, /MANDATORY_METHOD_STEP_XYZ/); assert.match(promptContext, /ResearchBundle with evidence references/)
