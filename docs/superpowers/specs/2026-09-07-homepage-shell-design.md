...
Build a focused, same-origin React client over the completed ResearchHub Runtime. The shell uses three persistent desktop regions: a conversation rail on the left, the active conversation in the center, and a compact context rail on the right.
...
- Knowledge search and object detail are read-only context views. Review Inbox is read-only and has no decision controls.
...
Add deterministic client tests for bootstrap, conversations, SSE resync, Knowledge, Review, attachments, production, Workflow, and no-Knowledge-Base behavior. Add Runtime static-serving tests and one faux-Pi local integration smoke test, then run the existing root suite unchanged.
