...
- Pi exposes governed `inspect_external_skill` and `install_external_skill` tools backed by `SkillOnboardingService`. Installation requires HTTPS GitHub plus a 40-character commit pin; unsafe Skills are inspectable but install-denied in V1 with `UNSAFE_SKILL_REQUIRES_MANUAL_TRUST_REVIEW`, without registry mutation or activation. Installed ResearchHub Skills enter the ResearchHub registry and remain separate from `.pi/skills/`.
- Research dispatch loads the authoritative `SKILL.md` methodology for every selected ResearchHub Skill from a bounded, validated source before a session starts; the loaded methodology is included in the Pi session context.
- Safe external ResearchHub Skills install under the ResearchHub runtime directory and are registered only after their installed `SKILL.md` is available. Safe `pi_native` Skills install into the project `.pi/skills/` tree, then use Pi's `DefaultResourceLoader.reload()` and `getSkills()` to verify discoverability; they never enter `ResearchSkillRegistry`. Unsupported external classifications fail explicitly rather than reporting activation.
...
- Free/Skill session prompts receive the selected ResearchHub Skill definitions (`intentDescription`, `whenToUse`, and `outputContract`), bounded authoritative methodology text, plus bounded Source Library provenance. The finalized session `ResearchBundle` records the answer, selected skills, hits, entities, evidence refs, and proposal candidates.
...
- `npm run acceptance:unified-research-final-fix` passes all F1–F7 cases and writes `docs/project-state/evidence/2026-09-17-unified-research-entry-final-fix.json`. The artifact proves built-in and safe external Research Skill methodology loading, missing-methodology fail-closed startup, removal of the Agent-settable unsafe-install bypass, unsafe inspect-only behavior, actual Pi-native `.pi/skills` activation with reload/discoverability and fresh-loader restart checks, and namespace separation.

## Closure status

- Original A–G, closure C1–C7, and Final Fix F1–F7 acceptance remain required release gates. Final branch state is verified by the delivery audit as `origin/main == HEAD` with a clean worktree.
