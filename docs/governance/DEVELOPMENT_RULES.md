...
58. Runtime v0.1 remains local-first, single-user, and loopback-bound by default.
59. Use one Node.js / TypeScript Application Runtime process; do not introduce microservices, Redis, queues, or worker clusters.
60. Embed Pi through the SDK; do not integrate Pi through an RPC subprocess.
61. Do not build ResearchHub conversation persistence; use Pi session infrastructure.
62. Use Pi AgentSessionRuntime for conversation lifecycle, with at most one active conversation runtime in v0.1.
63. Browser client technology is React + TypeScript + Vite.
64. Browser code must not access the filesystem, canonical Knowledge storage, or Writer directly.
65. Browser Product APIs must use the existing shared Application Services.
66. HTTP JSON + SSE is the current v0.1 transport; do not add WebSocket without a new architecture decision.
67. Do not stream raw model hidden reasoning, system prompts, credentials, raw stacks, or unrestricted internal tool payloads.
68. Upload creates an AttachmentRef only; upload is not canonical ingestion.
69. Browser-facing file identifiers must be attachment IDs, never arbitrary host filesystem paths.
70. Canonical Knowledge files must never be statically served.
71. Runtime mutation APIs require same-origin protection and a runtime nonce/token; do not add login, RBAC, JWT, or multi-user identity in v0.1.
72. Do not create a generic Event Bus or Workflow Engine for client streaming or observation.
73. Workflow progress must remain authoritative; Workflow polling is the current v0.1 observation design.

## 11. Governance and Git

74. `CURRENT_STATUS.md` is a compact current snapshot, not an append-only history.
75. Record only material architecture/product decisions in `DECISION_LOG.md`.
76. Update governance when the architecture or implementation state materially changes.
77. Every completed engineering task must be committed and pushed.
78. Return the exact commit hash in the engineering report.
79. A clean working tree is required at task completion.
80. Engineering completion does not equal CTO/System Architect acceptance.
