...
- `RHL-FIX-CLAIM-TEMPORAL-001` is implemented pending CTO acceptance: ClaimCandidate temporal typing and validation now align with the existing Schema 0.3 `Date.parse` admissibility semantics, semantic period labels remain in `scope.label`, malformed temporal Claims are isolated at Candidate validation and by a Planner defense-in-depth guard, and Writer/Knowledge Resolution/Schema semantics remain unchanged. R5 remains historical `PRODUCT_DEFECT` evidence; no real R6 was run or authorized.
