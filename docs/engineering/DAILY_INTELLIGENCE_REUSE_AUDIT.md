...
FIX-001 verification additionally distinguishes catalog count from active operational count, records per-provider discover/fetch/normalize/usable stages, rejects homepage-only candidates, treats Xueqiu authentication as blocked, and retains provider limitations as explicit evidence rather than fabricated success. FIX-002 adds the narrow `createDailyIntelligenceComposition` shared by Runtime, CLI, Scheduler, and Pi, plus real semantic-operation telemetry and canonical-only report provenance.
