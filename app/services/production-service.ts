...
export class ProductionService {
  ...
  async ingestDocument(input: IngestDocumentInput, callerSignal?: AbortSignal): Promise<ApplicationProductionResult> {
    ...
    const documentInput = hasText ? { type: 'text' as const, text: input.text!, originalFilename: input.originalFilename ?? 'researchhub-prompt.txt', mediaType: input.mediaType ?? 'text/plain' } : { type: 'file' as const, reference: await this.resolveWorkspaceFile(input.workspaceFile!) }
    ...
    const result = this.options.workflowService.start(input.workflowRunId, async (activeSignal): Promise<WorkflowOutcome & { readonly workflow: IngestionWorkflowResult }> => {
