...
export class ResearchHubRuntimeServer {
  ...
  private async route(request: IncomingMessage, response: ServerResponse, url: URL): Promise<void> {
    ...
    if (method === 'GET' && path.startsWith('/api/research-reports/')) { const reportId = decodeSegment(path.split('/')[3] ?? ''); const service = this.runtime!.researchService; if (!service) throw new ApplicationServiceError('not_found', 'Company Research is not configured'); await this.sendJson(response, 200, await service.getResearchReport(reportId)); return }
