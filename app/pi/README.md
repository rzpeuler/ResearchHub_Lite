...
An optional Codex CLI completion backend is available through the explicit `createCodexCliLunaReasoningExecutor()` factory in `app/pi/model-selection.ts`. It pins `gpt-5.6-luna` with medium reasoning, sends the Pi-built semantic context through stdin, converts ResearchHub output contracts to Codex transport JSON Schema and requires every declared object property at generation time while keeping the original ResearchHub contract and validators authoritative, uses `codex exec` with ephemeral read-only sandboxing and no approval escalation, and returns through the existing Pi JSON contract. It is an external completion backend behind the Pi reasoning boundary, is not a native `ModelRuntime` model, and is not the production default; callers must opt in explicitly.

The session registers the eight core product-level ResearchHub Application Tools: `researchhub_status`, `search_knowledge`, `get_knowledge_object`, `ingest_document`, `get_workflow_status`, `cancel_workflow`, `list_review_cases`, and `get_review_case`, plus configured research and Daily Intelligence actions when their services are present. They call the thin services under `app/services/`; no low-level canonical mutation tool or `resolve_review_case` is exposed. A mounted Knowledge Base root is fixed for the session; callers cannot override it through tool parameters. `workspaceRoot` defaults to `<cwd>/workspace` for file-based `ingest_document` input.
