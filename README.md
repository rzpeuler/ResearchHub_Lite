...
Personal Research v1 Company Research foundation is implemented on an explicit Schema 0.4 / Storage 1 fresh-KB lane; the foundation fix is implemented pending independent CTO acceptance. The `research_company` Application Service, Pi tool, and optional HTTP Product API run bounded official-disclosure, AKShare, GDELT, and RSS acquisition, produce a Markdown report and non-canonical ResearchSignals, then pass semantic proposals through the producer-neutral Knowledge Production Gateway, runtime-validated ChangeSet, and shared Writer. See [Personal Research v1 Architecture](docs/architecture/PERSONAL_RESEARCH_V1_ARCHITECTURE.md) and [Reuse Audit](docs/engineering/PERSONAL_RESEARCH_V1_REUSE_AUDIT.md).
