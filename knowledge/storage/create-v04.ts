...
export async function createFreshKnowledgeBaseV04(rootRef: string, options: { readonly knowledgeBaseId?: string; readonly name?: string; readonly now?: string } = {}): Promise<string> {
  ...
  for (const directory of ['registry', 'raw', 'theme-groups', 'entities', 'relations', 'claims', 'sources', 'modules', 'events', 'observations', 'theses', 'reasoning-edges', 'logs/research']) await mkdir(join(root, directory), { recursive: true })
