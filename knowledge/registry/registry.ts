...
export class KnowledgeBaseRegistry {
  ...
  async mount(rootRef: string): Promise<KnowledgeBaseHandle> {
    ...
    if (!['0.3', '0.4'].includes(manifest.schemaVersion) || manifest.storageFormatVersion !== '1') throw new KnowledgeError('UnsupportedSchema', `Unsupported Knowledge Base version: ${manifest.schemaVersion}/${manifest.storageFormatVersion}`, rootRef)
  ...
  register(handle: KnowledgeBaseHandle): KnowledgeBaseHandle {
    if (!['0.3', '0.4'].includes(handle.schemaVersion) || handle.storageFormatVersion !== '1') throw new KnowledgeError('UnsupportedSchema', 'Only Schema 0.3 or 0.4 / Storage Format 1 Knowledge Bases can be mounted')
